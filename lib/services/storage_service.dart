...
///
/// Unlike Firebase Storage, Cloudinary returns a permanent public URL at
/// upload time, so that URL is stored directly as `audioPath`/`coverUrl`
/// in Firestore — no separate "resolve download URL" step is needed.
class StorageService {
  ...
  Future<String> _upload(File file, String resourceType, String publicId, {
    ...
    onProgress?.call(0.1); // Cloudinary's simple upload API doesn't expose
    // byte-level progress without a lot of extra plumbing, so we just show
    // "in progress" rather than a precise percentage.
    ...
    final match = RegExp(r'"secure_url":"([^"]+)"').firstMatch(response.body);
    if (match == null) {
  ...
  Future<String> uploadAudio(String songId, File file, {
    void Function(double progress)? onProgress,
  }) {
  ...
  /// Kept for interface compatibility — Cloudinary URLs are already
  /// permanent public links, so this just returns the stored value as-is.
  Future<String> getDownloadUrl(String storedUrl) async => storedUrl;
}
